...
function ScatterChart (props) {
    ...
    // const independentVar = props.dataset.axes.x
    // const dependentVar = props.dataset.axes.y
    ...
    React.useEffect(() => {
        ...
        // console.log(...data.map(d=>Math.ceil(d.population)) + " what is d population?")
        // console.log(dependentVar + "this is y?")
        const max = Math.max(...data.map(d=>Math.ceil(Object.values(d)[1]))); //revisit, instead of population it is y axis
        // console.log(max + " What is this max variable?, its actually the length of said data")
        ...
        const getBarYValue = item => config.labels.y - (Object.values(item)[1] - config.data.min) * config.bars.height / dataRange; //revisit, instead of population it is y axis
        // const getBarWidth = () => config.bars.ratio * config.bars.width / (data.length + 1 - config.bars.ratio);
        const getBarHeight = item => (Object.values(item)[1] - config.data.min) * config.bars.height / dataRange; //revisit, instead of population it is y axis
        ...
        svgRef.current.select("#labels")
                ...
                .join(
                    enter => {
                        ...
                        .attr('y', getLabelY())
                        .text((item, index) => {
                            // return item.year; //this needs to be revisted, instead of year its x axis
        ...
        svgRef.current.select("#values")
            ...
            .join(
                enter => {
                    enter
                        .append("circle")
                        .attr("cx", (item , index) => getBarXValue(index)+cirlceDifference)
                        .attr("cy", (item , index) => getBarYValue(item))
                        .attr("r", 1)
                        .attr("height", (item, index) => getBarHeight(item))                    
                },
                update=> {
